...
Original file is located at
    https://colab.research.google.com/drive/1NSXdAKJkO8GvvaaVjS__FqfUQ08xJDTQ
"""
...
# in our dataset which is bpi maximum of 13 element was found in a trace
#the padding is done so that operations could be easily performed
#we calculated maxlen earlier, it gave 13 as answer for this dataset
...
#concatinating other features(ofea) into the one-hot encoding making the shape(-1,13,10) for this dataset.
...
#similar to zerofirst but zerotest is for the ground truth suffix extracted earlier.
zerotest = []
for li in preact:
  for i in range(len(li)):
    if (li[i] == 0 or i == len(li)-1):
